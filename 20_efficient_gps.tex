...
This approach represents uncertainty about the correct choice of parameters but assumes that the predefined structure of the function is correct, making it a parametric model.
...
\todo[inline]{No woodbury?}
Let $\mat{A} \in \Rb^{N \times N}$, $\mat{U} \in \Rb^{N \times K}$, $\mat{B} \in \Rb^{K \times K}$ and $\mat{V} \in \Rb^{K \times N}$ with invertible $\mat{A}$ and $\mat{B}$. Then the identity
\begin{align}
    \label{eq:matrix_inversion_lemma}
    \big(\mat{A} - \mat{U}\mat{B}\mat{V}\big)\inv
    &= \mat{A}\inv - \mat{A}\inv\mat{U}\big(\mat{B}\inv + \mat{V}\mat{A}\inv\mat{U}\big)\inv \mat{V}\mat{A}\inv
\end{align}
is called the matrix inversion lemma or Woodbury identity~\parencite{petersen_matrix_2008,higham_accuracy_2002}.

The density of $\mat{x} \sim \Gaussian{\mat{\mu}, \mat{\Sigma}}$ is
\begin{align}
\begin{split}
...
\end{split}
\end{align}
then the marginal distribution is a Gaussian with
\begin{align}
...
\end{align}
and the conditional is a Gaussian as well with
\begin{align}
\begin{split}
    \label{eq:gaussian_conditional}
    \Prob*{\mat{x}_1 \given \mat{x}_2}
    &= \frac{\Prob*{\mat{x}_1, \mat{x}_2}}{\Prob*{\mat{x}_2}} \\
    &= \Gaussian{\hat{\mat{\mu}}, \hat{\mat{\Sigma}}}\text{, with} \\[\smallskipamount]
    \hat{\mat{\mu}}
...
\Cref{fig:gp:gp_samples} compares sample functions drawn from Gaussian processes with the linear kernel and squared exponential kernels with different hyper-parameters.
...
\Cref{fig:gp:gp_posterior} shows samples from a pair of prior and posterior Gaussian processes.
...
In a fully Bayesian setup, the correct way to model uncertainty about hyper-parameters is to assign them a prior $\Prob{\mat{\theta}}$ and marginalize it to derive the dependent distributions
\begin{align}
...
\begin{figure}[tp]
    ...
    \caption[SPGP example]{
        ...
        \Cref{fig:sparse_gp:spgp_example:spgp} shows an SPGP with pseudo inputs located at the dart positions.
...
The predictive distribution as written in the previous equations can easily be compared to the predictive posterior of Gaussian processes in \cref{lem:gp:gp_posterior}.
...
\Cref{fig:sparse_gp:spgp_example} shows that a surprisingly small number of pseudo inputs can be enough to represent the dynamics of a function.

With a large number of pseudo inputs, the number of hyper-parameters can grow large.
...
\begin{figure}[t]
    \begin{subfigure}[b]{\halffigurewidth}
        ...
        \begin{tikzpicture}[x=4em, y=5em]
            ...
            \draw[edge, directed] (Z) -- ++(-2.25em, 0) -- (F);
            %
            \begin{scope}[on background layer]
                \node[
                    align plate,
