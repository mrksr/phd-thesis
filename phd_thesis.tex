...
\begin{definition}[Probabilistic Numerical Method]
    ...
    Let $\Uc$, $\Yc$ and $\Qc$ be measurable spaces and let $Y: \Uc \to \Yc$, $Q: \Uc \to \Qc$ and $B: \Probs{\Uc} \times \Yc \to \Probs{\Qc}$ where $Y$ and $Q$ are measurable functions.
    The pair $M = (Y, B)$ is called a \emph{probabilistic numerical method} for estimation of a quantity of interest $Q$.
    The map $Y$ is called an \emph{information opertor} and the map $B$ is called a \emph{belief update operator}.
\end{definition}
