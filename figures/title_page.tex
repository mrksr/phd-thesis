...
\begin{tikzpicture}[x=\figurewidth, y=\figureheight, every node/.style={inner sep=0}]
    ...
    \tikzstyle{author} = [font=\Large\bfseries]
    \tikzstyle{title} = [sPetrol, font=\Huge\bfseries]
    \tikzstyle{title bar} = [title, line width=2pt, line cap=round]

    \node[anchor=east, date] (date) at (0.95, 0.9) {Submitted on \makeatletter\@date\makeatother};
    ...
    \coordinate (title height) at ($(date.south east)!0.5!(chairs.north east)$);
    \node[anchor=east] (title) at (title height -| 1,0) {
        \begin{tikzpicture}[x={(1.0,0)},y={(0,0.075)}]
            ...
            \node[author] (author) at (0.95, -0.25) {\makeatletter\@author\makeatother};
        \end{tikzpicture}
    };
