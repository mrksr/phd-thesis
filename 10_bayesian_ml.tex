...
\end{labeling}
\end{problem}
\Cref{fig:bayesian_ml:polynoms:linear} shows the results of three algorithms with different choises of error functions $e$.
With $f(x) = \mat{W}x + b$ those are
\begin{labeling}{Least Absolute Error:}
    \item[Least Squares:] $e(f, \Dc) = \sum_{n=1}^N (y_n - f(x_n))^2$
    \item[Least Absolute Error:] $e(f, \Dc) = \sum_{n=1}^N \abs{y_n - f(x_n)}$
...
\section{Industrial applications of machine learning}
